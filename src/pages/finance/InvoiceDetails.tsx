...
const InvoiceDetails: React.FC<{ invoice: Invoice }> = ({ invoice }) => {
  ...
  const fetchTransactionDetails = async () => {
    try {
      const response = await transactionApi.findTransactionWithInvoice(
        Number(id)
      );
  ...
  const fetchInsuranceClaim = async () => {
    try {
      const response = await invoiceApi.findMedishieldClaimOfInvoice(
        Number(id)
      );
  ...
  const fetchTransactionClaim = async () => {
    try {
      const response = await invoiceApi.findTransactionItemOfInvoice(
        Number(id)
      );
  ...
  const handleToken = async (token: any) => {
    console.log(token);
    console.log(
      transactionItem.reduce(
        (total, item) => total + item.transactionItemPrice,
        0
      )
    );
    ...
    try {
      try {
        const response2 = await axios.post(
          `${REST_ENDPOINT}/api/payment/charge`,
          "",
          {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("accessToken"),
              token: token.id,
              amount: transactionItem.reduce(
                (total, item) =>
                  total +
        ...
        if ((error as any).response && (error as any).response.status === 400) {
          ...
          if (
            errorMessage.includes("Error Encountered: Your card was declined")
          ) {
            const total = transactionItem.reduce(
              (total, item) =>
                total +
                item.transactionItemPrice * item.transactionItemQuantity,
              0
            ...
            const response = await transactionApi.createFailedTransaction(
              Number(id),
              total
            );
            console.log(response.data);
  ...
  return (
    <IonPage>
      ...
      <IonContent fullscreen>
        <IonCard>
          <IonCardHeader>Invoice ID: {state?.invoiceId}</IonCardHeader>
          <IonCardContent>
                      ...
                      {transactionItem
                        ...
                        .map((item) => (
                          <IonItem key={item.transactionItemId}>
                            <IonLabel>
                              <h2>{item.transactionItemName}</h2>
                              <p>${item.transactionItemPrice.toFixed(2)}</p>
                            </IonLabel>
                          </IonItem>
                        ))}
                    </IonList>
